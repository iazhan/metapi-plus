...
本文档详细说明如何将不同类型的 AI 中转站、官方 API 与兼容入口接入 Metapi Plus。
...
1. **登录管理后台** — 访问 `http://your-metapi-host:4000`，使用 `AUTH_TOKEN` 登录 Metapi Plus
2. **进入站点管理** — 点击左侧菜单「站点管理」
3. **添加站点** — 点击「添加站点」按钮
4. **填写站点信息** — 根据站点类型填写对应字段；如果是官方入口，也可以直接选择预设
5. **添加连接** — 面板型站点添加账号 / Session，兼容接口和官方 API 添加 API Key
...
添加或编辑站点时，自动识别会直接使用表单中尚未保存的站点代理设置。识别失败不会覆盖已有站点配置。

#### 站点模型别名

编辑已有站点时，可在「模型别名」区域把真实来源模型额外暴露为更稳定或更便于团队使用的模型名。例如把 `gpt-4o` 暴露为 `team-fast`。

- 原始模型和别名会同时出现在可用模型列表中。
- 别名只改变下游请求名称；上游请求、计费和日志仍使用真实来源模型。
- 同一别名可在多个站点聚合通道，但所有站点必须使用完全一致的大小写和拼写，单个站点内也不能重复。
- 禁用站点、禁用来源模型、全局模型白名单和下游密钥权限仍然生效，别名不能绕过这些限制。
- 别名路由在路由管理中只读；修改或删除别名应回到站点编辑页。
...
- **填写方式：**
  - 用户名：`your-username`
  - 密码：`your-password`
- **自动获取：** 系统自动登录并获取 Access Token、站点返回的真实 User ID、账号令牌和可用分组倍率
...
**User ID 自动获取：** New API 通常需要在请求头中携带 `New-API-User` / `Veloera-User` / `voapi-user` 等字段。Metapi Plus 会按以下顺序获取：

1. 优先使用账号密码登录响应返回的真实 User ID
2. 从 JWT Token 中解码 User ID
3. 从 Session Cookie 中提取 User ID（支持 Gob 编码解析）
4. 通过兼容探测验证可用性

如果以上方法都不能获取到 ID，则需要用户手动获取。

**挑战边界：** 当前保留 ACW Shield 一类 JS 挑战（`acw_sc__v2` / `cdn_sec_tc`）兼容。Turnstile 和登录 2FA 不接入第三方代解服务；遇到这两类验证时，Metapi Plus 会返回明确错误，需要先在原站完成验证再使用 Session 凭证。
...
**额外功能：** OneHub 支持 Token 分组（`token_group`），Metapi Plus 会自动识别并保留分组信息。
...
- Veloera 需要 `Veloera-User` 请求头，Metapi Plus 会自动添加
...
Sub2API 使用 JWT 短期会话机制，和传统 NewAPI 站点差异较大。Metapi Plus 支持以下两种添加方式：

##### 1. 用户名密码登录

1. 在账号添加界面选择用户名密码方式
2. Metapi Plus 通过 Sub2API 的 `/api/v1/auth/login` 登录
3. 登录成功后会保存 Access Token、Refresh Token 和过期时间，并用同一会话初始化账号令牌与分组倍率
4. 如果站点要求 2FA 或 Turnstile，Metapi Plus 不会调用第三方打码平台；请先在原站完成验证，再改用下方 Session 方式
...
3. 若 `GET /v1/models` 为空，先确认该账号下已有可用用户 API Key，Metapi Plus 会再尝试用它发现模型

---

### 账号分组倍率同步

New API 和 Sub2API 账号在同步站点令牌时，会同时同步该账号当前可用的分组倍率。Token 页面会显示类似 `VIP · 0.8x` 的标签。
...
- 倍率按账号保存，不会复制到每一条 Token
- 自动刷新默认开启，间隔只接受 `5` 到 `10080` 的整数分钟
- 如果你已经在管理后台「设置」里保存过账号分组倍率刷新配置，UI 持久化值会覆盖环境默认值
- 服务启动时，或者你把该开关从关闭切回开启时，系统会立即执行一次**仅倍率刷新**
- 自动刷新只更新账号分组倍率快照，**不会**同步 Token、余额、模型或路由
- 同步成功返回空倍率列表时，会清理该账号之前的倍率快照
- 倍率接口失败时，Token 同步结果仍然保留，同时继续显示上一次成功的倍率快照
- 如果账号 Session 已过期，系统会先尝试 `Refresh Token`，不行时再尝试用加密保存的账号密码恢复登录
- 分组倍率只是上游计费中的一层系数，不代表完整模型价格，不能替代模型定价配置
...
**协议转换：** Metapi Plus 自动处理 OpenAI 格式与 Claude Messages API 格式的双向转换，下游客户端可使用 OpenAI SDK 调用 Claude 模型。
...
**协议转换：** Metapi Plus 自动处理 OpenAI 格式与 Gemini `generateContent` API 格式的双向转换。
...
2. Metapi Plus 会向该 URL 发送 POST 请求执行签到
3. 请求头自动携带账号凭证
...
Metapi Plus 支持自动识别站点类型，当前检测优先级如下：
...
Metapi Plus 会定期抓取已接入站点的公告，并在首次发现时写入站内通知与「站点公告」页面。
...
1. 首次发现的公告会触发一次 Metapi Plus 通知
2. 已经保存过的公告再次同步时只更新本地记录
3. `sub2api` 这类需要登录态的公告接口，会优先使用站点下已启用账号的会话令牌
4. 「站点公告」页面的清空操作只影响 Metapi Plus 本地数据库
...
1. Metapi Plus 会自动探测 User ID，通常无需手动配置
...
**原因：** `cliproxyapi` / CPA 在 Metapi Plus 里按标准 API provider 处理，重点支持模型发现与代理调用，不按传统面板站处理。
...
- **国内访问 OpenAI / Claude / Gemini：** 通常需要代理
- **国内中转站：** 通常不需要代理
- **海外中转站：** 根据网络情况选择
