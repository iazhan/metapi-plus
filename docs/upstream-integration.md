...
本文档详细说明如何将不同类型的 AI 中转站、官方 API 与兼容入口接入 Metapi。
...
**User ID 自动获取：** New API 通常需要在请求头中携带 `New-API-User` / `Veloera-User` / `voapi-user` 等字段。Metapi 会按以下顺序获取：
...
**挑战边界：** 当前保留 ACW Shield 一类 JS 挑战（`acw_sc__v2` / `cdn_sec_tc`）兼容。Turnstile 和登录 2FA 不接入第三方代解服务；遇到这两类验证时，Metapi 会返回明确错误，需要先在原站完成验证再使用 Session 凭证。
...
Sub2API 使用 JWT 短期会话机制，和传统 NewAPI 站点差异较大。Metapi 支持以下两种添加方式：

##### 1. 用户名密码登录

1. 在账号添加界面选择用户名密码方式
2. Metapi 通过 Sub2API 的 `/api/v1/auth/login` 登录
3. 登录成功后会保存 Access Token、Refresh Token 和过期时间，并用同一会话初始化账号令牌与分组倍率
4. 如果站点要求 2FA 或 Turnstile，Metapi 不会调用第三方打码平台；请先在原站完成验证，再改用下方 Session 方式
