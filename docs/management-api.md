...
本文档说明如何直接调用 Metapi Plus 管理后台的 `/api/*` 接口，用脚本完成站点与账号管理。
...
- 在外部系统里接入 Metapi Plus 后台能力，而不是手工点页面
...
- `AUTH_TOKEN` 就是你启动 Metapi Plus 时配置的管理员令牌
...
| `platformUserId` | `number` | 否 | 某些 `new-api` 兼容站点需要手动传 |
...
> [!TIP]
> 对 `new-api` 兼容站点，如果返回 `needsUserId: true`，说明脚本里要补上 `platformUserId` 再重试。
