...
## 账号认证方式与兼容性（Session / API Key）

### Q: Access Token、Cookie、API Key 应该选哪个？

**A:** 先按上游类型选择：

| 方式 | 特点 | 适用场景 |
|------|------|----------|
| **Access Token（系统访问令牌）** | 稳定、适合长期使用和多账号 | New API / One API / OneHub / DoneHub / Veloera 等面板站 |
| **Cookie（浏览器登录态）** | 兼容性强，但更容易过期 | Access Token 不可用、部分面板变体站点、特殊兜底场景 |
| **API Key** | 配置最简单，直接用于代理调用 | OpenAI / Claude / Gemini 兼容入口、CPA、官方预设 |
...
**A:** 这类站点走 `cliproxyapi` 平台类型，推荐直接添加 API Key，不要把它当成可签到、可登录的面板站。详细步骤见 [上游接入](/upstream-integration)。
...
- 精确模型权限（直接授权对外模型名和站点模型别名）
- 路由群组权限（授权通配符、正则或显式群组路由）
- 站点倍率（控制不同项目的上游偏好）
...
- [搜索已有 Issue](https://github.com/iazhan/metapi-plus/issues?q=is%3Aissue) — 看看是否有人遇到过相同问题
- [提交新 Issue](https://github.com/iazhan/metapi-plus/issues/new) — 报告 Bug 或提出功能建议
- [参与讨论](https://github.com/iazhan/metapi-plus/discussions) — 使用疑问、经验分享
- [文档首页](/) — 查看所有文档
