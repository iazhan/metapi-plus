...
本文档说明如何将下游客户端连接到 Metapi Plus 代理网关。
...
Metapi Plus 暴露标准 OpenAI / Claude 兼容接口，下游客户端通常只需配置两项：
...
> 说明：Metapi Plus 现在支持标准 OpenAI `/v1/files` 文件链路。
> Cherry Studio、Open WebUI 等客户端如果通过标准 OpenAI Files + `Responses` / Chat 文件块发送 PDF、Markdown、JSON、图片、音频附件，即可经由 Metapi Plus 转发。
...
| 变量 | 作用 |
|------|------|
| `ANTHROPIC_BASE_URL` | 指向 Metapi Plus 的根域名，Claude Code 会自动拼接 `/v1/messages` |
...
> 上述变量由 Claude Code 客户端读取，属于客户端行为开关；Metapi Plus 服务端只能处理已经发来的请求。
...
> 提示：`model` 需要是你在 Metapi Plus `GET /v1/models` 可见的模型名。
...
| 有 "Anthropic" 选项 | 填 `https://your-domain.com`，Metapi Plus 自动处理 `/v1/messages` |
...
除了全局 `PROXY_TOKEN`，你还可以创建多个项目级下游 Key，每个 Key 可独立配置归属分组、标签、过期时间、费用/请求上限、精确模型权限、路由群组权限和站点倍率，适用于多团队/多项目共用一个 Metapi Plus 实例的场景。
