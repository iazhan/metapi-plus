...
describe('platformIdentity', () => {
  it('normalizes shared platform aliases', () => {
    ...
    expect(normalizePlatformAlias('chatgpt-codex')).toBe('chatgpt-codex');
    expect(normalizePlatformAlias('anti-gravity')).toBe('anti-gravity');
  ...
  it('detects platform by well-known url hints', () => {
    ...
    expect(detectPlatformByUrlHint('https://chatgpt.com/backend-api/codex')).toBeUndefined();
    ...
    expect(detectPlatformByUrlHint('https://cloudcode-pa.googleapis.com')).toBeUndefined();
