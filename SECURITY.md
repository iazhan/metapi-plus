...
The security of Metapi Plus is important to us. This document outlines our security policy and how to report vulnerabilities.

Metapi Plus 的安全对我们很重要。本文档概述了我们的安全政策以及如何报告漏洞。

Since Metapi Plus is a self-hosted meta-aggregation layer that manages sensitive credentials (API keys, account passwords) and proxies AI API requests, security is a critical concern.

由于 Metapi Plus 是一个自托管的元聚合层，管理敏感凭证（API 密钥、账号密码）并代理 AI API 请求，因此安全性至关重要。
...
**Recommendation** / **建议**: Always use the latest stable release from [GitHub Releases](https://github.com/iazhan/metapi-plus/releases) or the `main` branch for the most up-to-date security patches.

始终使用 [GitHub Releases](https://github.com/iazhan/metapi-plus/releases) 的最新稳定版本或 `main` 分支以获得最新的安全补丁。

## Security Considerations / 安全注意事项

When deploying Metapi Plus, please consider / 部署 Metapi Plus 时，请考虑:
...
1. **GitHub Security Advisory** (Preferred) / **GitHub 安全公告**（首选）
   - Go to: https://github.com/iazhan/metapi-plus/security/advisories/new
   - This allows for coordinated disclosure and CVE assignment / 这允许协调披露和 CVE 分配

2. **Email** / **邮件**
   - Send to: `xigchn@163.com`
   - Subject: `[Metapi Plus Security] <brief description>` / 主题：`[Metapi Plus Security] <简要描述>`
...
```
Subject: [Metapi Plus Security] SQL Injection in account search
...
Reproduction:
1. Login to the Metapi Plus admin panel
...
- [GitHub Security Advisories](https://github.com/iazhan/metapi-plus/security/advisories)
- [GitHub Releases](https://github.com/iazhan/metapi-plus/releases) with `[SECURITY]` tag / 带有 `[SECURITY]` 标签
...
If you have questions about this security policy, please contact `xigchn@163.com`.

如果您对本安全政策有疑问，请联系 `xigchn@163.com`。

---

**Thank you for helping keep Metapi Plus and its users safe!**

**感谢您帮助保护 Metapi Plus 及其用户的安全！**
